...
import { is, map, repeat, zipWith } from "ramda";
import {
  isBoolExp,
  isCExp,
  isLitExp,
...
const evalIf = (exp: IfExp, env: Env): Result<Value> =>
  bind(applicativeEval(exp.test, env), (test: Value) =>
    isTrueValue(test) ? applicativeEval(exp.then, env) : applicativeEval(exp.alt, env)
  );

const evalProc = (exp: ProcExp, env: Env): Result<Closure> => makeOk(makeClosure(exp.args, exp.body, env));
...
export const evalSequence = (seq: Exp[], env: Env): Result<Value> =>
  isNonEmptyList<Exp>(seq) ? evalCExps(first(seq), rest(seq), env) : makeFailure("Empty program");

const evalCExps = (first: Exp, rest: Exp[], env: Env): Result<Value> =>
  isDefineExp(first)
    ? evalDefineExps(first, rest)
...
export const evalProgram = (program: Program): Result<Value> => evalSequence(program.exps, theGlobalEnv);

export const evalParse = (s: string): Result<Value> =>
  bind(p(s), (x) => bind(parseL4Exp(x), (exp: Exp) => evalSequence([exp], theGlobalEnv)));
...
const evalSet = (exp: SetExp, env: Env): Result<void> =>
  bind(applicativeEval(exp.val, env), (val: Value) =>
    mapv(applyEnvBdg(env, exp.var.var), (bdg: FBinding) => setFBinding(bdg, val))
  );

// HW3 complete this function.
const evalBound = (exp: BoundExp, env: Env): Result<boolean> =>
  isExtEnv(env)
    ? bind(evalBound(exp, env.env), (isBound: boolean) =>
        makeOk(isBound || env.frame.fbindings.find((bdg) => bdg.var === exp.var.var) !== undefined)
      )
    : makeOk(unbox(env.frame).fbindings.find((bdg) => bdg.var === exp.var.var) !== undefined);

// HW3 complete this function.
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> => {
  const beforeEvaluation = new Date();
  const evaluation = applicativeEval(exp.exp, env);
  const afterEvaluation = new Date();
  return bind(evaluation, (val: Value) =>
    makeOk(makeCompoundSExp(val, afterEvaluation.getTime() - beforeEvaluation.getTime()))
  );
};