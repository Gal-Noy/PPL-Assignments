...
;;         |  (bound? <var> )                 / BoundExp(var: varRef)                      #### HW3
;;         |  (time <cexp> )                  / TimeExp(exp: cexp)                         #### HW3
...
export const makeSetExp = (v: VarRef, val: CExp): SetExp => ({ tag: "SetExp", var: v, val: val });
export const makeBoundExp = (varRef: VarRef): BoundExp => ({ tag: "BoundExp", var: varRef });
export const makeTimeExp = (exp: CExp): TimeExp => ({ tag: "TimeExp", exp: exp });
...
export const parseL4SpecialForm = (op: SpecialFormKeyword, params: Sexp[]): Result<CExp> =>
  isNonEmptyList<Sexp>(params)
    ? op === "if"
      ...
      : op === "letrec"
      ? parseLetrecExp(first(params), rest(params))
      : op === "set!"
      ? parseSetExp(params)
      : op === "bound?"
      ? parseBoundExp(params)
        : op === "time"
        ? parseTimeExp(params)
...
export const parseDefine = (params: Sexp[]): Result<DefineExp> =>
  isNonEmptyList<Sexp>(params)
    ? params.length === 1
      ...
      : params.length > 2
      ? makeFailure(`define has too many arguments: ${format(params)}`)
...
const parseGoodDefine = (variable: Sexp, val: Sexp): Result<DefineExp> =>
  !isIdentifier(variable)
    ? makeFailure(`First arg of define must be an identifier: ${format(variable)}`)
    : mapv(parseL4CExp(val), (value: CExp) => makeDefineExp(makeVarDecl(variable), value));
...
export const parseL4Atomic = (token: Token): Result<CExp> =>
  token === "#t"
    ...
    : isString(token) && isPrimOpKeyword(token)
...
export const parseL4CExp = (sexp: Sexp): Result<CExp> =>
  isNonEmptyList<Sexp>(sexp)
    ? parseL4CompoundCExp(first(sexp), rest(sexp))
    : isToken(sexp)
    ? parseL4Atomic(sexp)
...
const parseAppExp = (op: Sexp, params: Sexp[]): Result<AppExp> =>
  bind(parseL4CExp(op), (rator: CExp) =>
    mapv(mapResult(parseL4CExp, params), (rands: CExp[]) => makeAppExp(rator, rands))
  );
...
const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> =>
  isArray(vars) && allT(isString, vars)
    ? mapv(mapResult(parseL4CExp, body), (cexps: CExp[]) => makeProcExp(map(makeVarDecl, vars), cexps))
    : makeFailure(`Invalid vars for ProcExp: ${format(vars)}`);

const isGoodBindings = (bindings: Sexp): bindings is [string, Sexp][] =>
  isArray(bindings) && allT(isNonEmptyList<Sexp>, bindings) && allT(isIdentifier, map(first, bindings));
...
const parseBindings = (bindings: Sexp): Result<Binding[]> => {
  ...
  return bind(valsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, vars, vals)));
};

const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> =>
  bind(parseBindings(bindings), (bindings: Binding[]) =>
    mapv(mapResult(parseL4CExp, body), (body: CExp[]) => makeLetExp(bindings, body))
  );

const parseLetrecExp = (bindings: Sexp, body: Sexp[]): Result<LetrecExp> =>
  bind(parseBindings(bindings), (bindings: Binding[]) =>
    mapv(mapResult(parseL4CExp, body), (body: CExp[]) => makeLetrecExp(bindings, body))
  );

const parseSetExp = (params: Sexp[]): Result<SetExp> =>
  isNonEmptyList<Sexp>(params)
    ? params.length === 1
      ? makeFailure(`set! missing 1 argument: ${format(params)}`)
      : params.length > 2
      ? makeFailure(`set! has too many arguments: format(params)`)
...
const parseGoodSetExp = (variable: Sexp, val: Sexp): Result<SetExp> =>
  !isIdentifier(variable)
    ? makeFailure(`First arg of set! must be an identifier: ${format(variable)}`)
...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
  isNonEmptyList<Sexp>(params)
    ...
    : makeFailure(`bound? has too many arguments`) :
    makeFailure("bound? missing 1 argument");

const parseGoodBoundExp = (variable: Sexp): Result<BoundExp> =>
    !isIdentifier(variable)
    ? makeFailure(`First arg of bound? must be an identifier: ${format(variable)}`)
    : makeOk(makeBoundExp(makeVarRef(variable)));

const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
    isNonEmptyList<Sexp>(params) ?
    params.length === 1 ? parseGoodTimeExp(first(params))
    : makeFailure(`time has too many arguments`) :
     makeFailure("time missing 1 argument");

const parseGoodTimeExp = (variable: Sexp): Result<TimeExp> =>
    bind(parseL4CExp(variable), (val: CExp) => makeOk(makeTimeExp(val)));
...
export const makeDottedPair = (sexps: Sexp[]): Result<SExpValue> =>
  bind(parseSExp(sexps[0]), (val1: SExpValue) =>
    mapv(parseSExp(sexps[2]), (val2: SExpValue) => makeCompoundSExp(val1, val2))
  );
  ...
  sexp === "#t"
    ...
    ? // fail on (x . y z)
      ...
      : bind(parseSExp(first(sexp)), (val1: SExpValue) =>
          mapv(parseSExp(rest(sexp)), (val2: SExpValue) => makeCompoundSExp(val1, val2))
        )
...
const unparseLetExp = (le: LetExp): string => `(let (${unparseBindings(le.bindings)}) ${unparseLExps(le.body)})`;

const unparseLetrecExp = (le: LetrecExp): string =>
  `(letrec (${unparseBindings(le.bindings)}) ${unparseLExps(le.body)})`;

const unparseSetExp = (se: SetExp): string => `(set! ${se.var.var} ${unparse(se.val)})`;
...
export const unparse = (exp: Parsed): string =>
  isBoolExp(exp)
    ...
    ? unparseProcExp(exp)
    : isIfExp(exp)
    ? `(if ${unparse(exp.test)} ${unparse(exp.then)} ${unparse(exp.alt)})`
    : isAppExp(exp)
    ? `(${unparse(exp.rator)} ${unparseLExps(exp.rands)})`
    : isPrimOp(exp)
    ? exp.op
    : isLetExp(exp)
    ? unparseLetExp(exp)
    : isLetrecExp(exp)
    ? unparseLetrecExp(exp)
    : isSetExp(exp)
    ? unparseSetExp(exp)
    : isBoundExp(exp)
    ? `(bound? ${unparse(exp.var)})`
    : isTimeExp(exp)
    ? `(time ${unparse(exp.exp)})`
    : isDefineExp(exp)
    ? `(define ${exp.var.var} ${unparse(exp.val)})`
    : isProgram(exp)
    ? `(L4 ${unparseLExps(exp.exps)})`
    : exp;
