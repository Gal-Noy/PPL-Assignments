...
export const checkCompatibleType = (
  te1: TExp,
  te2: TExp,
  exp: Exp
): Result<true> =>
  equals(te1, te2)
    ? makeOk(true)
    : isUnionTExp(te2)
    ? checkSubType(te1, te2, exp)
    : isProcTExp(te1) && isProcTExp(te2)
    ? checkCompatibleProcType(te1, te2, exp)
...
const checkSubType = (te1: TExp, te2: UnionTExp, exp: Exp): Result<true> =>
  isUnionTExp(te1)
    ? checkSubTypeUnion(te1, te2, exp)
    : te2.components.some((te: TExp) => isOk(checkCompatibleType(te1, te, exp)))
    ? makeOk(true)
    : bind(unparseTExp(te1), (te1: string) =>
        bind(unparseTExp(te2), (te2: string) =>
          bind(unparse(exp), (exp: string) =>
...
const checkSubTypeUnion = (
  te1: UnionTExp,
  te2: UnionTExp,
  exp: Exp
): Result<true> =>
  te1.components.every((te: TExp) => isOk(checkSubType(te, te2, exp)))
    ? makeOk(true)
    : bind(unparseTExp(te1), (te1: string) =>
        bind(unparseTExp(te2), (te2: string) =>
          bind(unparse(exp), (exp: string) =>
...
const checkCompatibleProcType = (
  ...
  if (te1.paramTEs.length !== te2.paramTEs.length)
    return makeFailure(
      `Incompatible types: ${unparseTExp(te1)} and ${unparseTExp(
        te2
      )} in ${unparse(exp)}`
    );
  const constraint1 = checkCompatibleType(te1.returnTE, te2.returnTE, exp);
  const constraint2 = te2.paramTEs.every((te: TExp, i: number) =>
    isOk(checkCompatibleType(te, te1.paramTEs[i], exp))
  );
...
export const L5typeof = (concreteExp: string): Result<string> =>
  bind(p(concreteExp), (x) =>
    bind(
      Array.isArray(x) && x[0] === "L5" ? parseL5Program(x) : parseL5Exp(x),
      (e: Exp | Program) => bind(typeofExp(e, makeEmptyTEnv()), unparseTExp)
    )
  );
...
export const makeUnion = (te1: TExp, te2: TExp): TExp => {
  const components = sort(
    (t_1: TExp, t_2: TExp) =>
      t_1.tag < t_2.tag ? -1 : t_1.tag > t_2.tag ? 1 : 0,
    union(
      isUnionTExp(te1) ? te1.components : [te1],
      isUnionTExp(te2) ? te2.components : [te2]
    )
  );
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
  ...
  const constraint2 = isOk(
    bind(thenTE, (thenTE: TExp) =>
      bind(altTE, (altTE: TExp) => checkCompatibleType(thenTE, altTE, ifExp))
    )
  );
  const constraint3 = isOk(
    bind(altTE, (altTE: TExp) =>
      bind(thenTE, (thenTE: TExp) => checkCompatibleType(altTE, thenTE, ifExp))
    )
  );
  return bind(constraint1, (_c1: true) =>
    safe2((then: TExp, alt: TExp) =>
      constraint2 ? altTE : constraint3 ? thenTE : makeOk(makeUnion(then, alt))
    )(thenTE, altTE)
  );
};
...
export const typeofLetrec = (exp: LetrecExp, tenv: TEnv): Result<TExp> => {
  ...
  const constraints = bind(types, (types: TExp[]) =>
    zipWithResult(
      (typeI, ti) => checkCompatibleType(typeI, ti, exp),
      types,
      tis
    )
  );
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> => {
  ...
  const valTE = exp.val;
  const constraint = bind(typeofExp(valTE, tenv), (typeOfVal: TExp) =>
...
export const typeofProgram = (exp: Program, tenv: TEnv): Result<TExp> =>
  isEmpty(exp.exps)
    ? makeFailure("Program not (L5 <exp>+ )")
    : evalTypeOfProgram(exp.exps, tenv);

export const evalTypeOfProgram = (exps: Exp[], tenv: TEnv): Result<TExp> => {
  const firstExp = exps[0];
  const restExp = exps.slice(1);
  return isEmpty(restExp)
    ? typeofExp(firstExp, tenv)
    : bind(typeofExp(firstExp, tenv), (_) =>
        evalTypeOfProgram(
          restExp,
