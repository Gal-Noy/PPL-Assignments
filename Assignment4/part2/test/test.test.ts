...
import { delayedSum, Post, postsUrl, postUrl, invalidUrl, fetchData, fetchMultipleUrls } from "../src/part2";
...
describe("Assignment 4 Part 2", () => {
  ...
  describe("Q2.2 fetchData (12 points)", () => {
    test("successful call to fetchData with array result", async () =>
      fetchData(postsUrl).then((res) => expect(res.length).toBe(100)));
  ...
  describe("Q2.3 fetchMultipleUrls (12 points)", () => {
    test("successful call to fetchMultipleUrls", async () =>
      fetchMultipleUrls(Array.from({ length: 20 }, (_, i) => i + 1).map((i) => postUrl + i)).then((res) =>
        expect(res.length).toBe(20)
      ));

    test("successful call to fetchMultipleUrls: verify results are in the expected order ", async () => {
      fetchMultipleUrls(Array.from({ length: 20 }, (_, i) => i + 1).map((i) => postUrl + i)).then((res) =>
        expect(res.map((post: Post) => post.id)).toEqual(Array.from({ length: 20 }, (_, i) => i + 1))
      );
    });

    test("failed call to fetchMultipleUrls", async () => {
      fetchMultipleUrls(Array.from({ length: 101 }, (_, i) => i + 1).map((i) => postUrl + i))
        .then((res) => expect(true).toEqual(false))
