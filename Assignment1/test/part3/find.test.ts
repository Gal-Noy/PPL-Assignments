...
describe("Find", () => {
  describe("findResult", () => {
    it("returns a Failure when no element was found", () => {
      ...
      expect(F.findResult((x: string) => x.length > 3, my_list)).toSatisfy(
        R.isFailure
      );
    });
    it("returns a Failure when no element was found", () => {
      const my_list: string[] = ["dog", "cat", "rat"];
      expect(F.findResult((x: string) => x[0] === "z", my_list)).toSatisfy(
        R.isFailure
      );
    });
    ...
    it("returns an Ok when an element was found", () => {
      const my_list: string[] = ["raccoon", "ostrich", "slug"];
      expect(F.findResult((x: string) => x.length > 3, my_list)).toEqual(
        R.makeOk("raccoon")
      );
    });
    it("returns an Ok when an element was found", () => {
      const my_list: string[] = ["raccoon", "ostrich", "slug"];
      expect(F.findResult((x: string) => x[0] === "s", my_list)).toEqual(
        R.makeOk("slug")
      );
    });
  });
  ...
  describe("returnSquaredIfFoundEven", () => {
    ...
    it("returns an Ok of the first even number squared in v2", () => {
      expect(F.returnSquaredIfFoundEven_v2([32, 64, 128])).toEqual(
        R.makeOk(1024)
      );
    });
