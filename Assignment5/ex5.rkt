...
(define equal-trees$ 
  (lambda (tree1 tree2 succ fail)
    (if (and (empty? tree1) (empty? tree2))
        (succ '())
        (if (and (empty? tree1) (not(empty? tree2)))
            (fail (cons tree1 tree2))
            (if (and (empty? tree1) (not(empty? tree2)))
                (fail (cons tree1 tree2))
                (if (and (leaf? tree1) (leaf? tree2))
                    (succ (cons tree1 tree2))
                    (if (and (leaf? tree1) (not(leaf? tree2)))
                        (fail (cons tree1 tree2))
                        (if (and (not (leaf? tree1)) (leaf? tree2))
                            (fail (cons tree1 tree2))
                            (equal-trees$ (car tree1) (car tree2) (lambda (car-res)
                                                                    (equal-trees$ (cdr tree1) (cdr tree2) (lambda (cdr-res)
...
(define diag
  (lambda (lzl)
    (letrec ((next-lzl (lambda (idx curr-lzl)
                         (cons-lzl (nth (head curr-lzl) idx)
                                   (lambda ()
                                     (next-lzl (+ idx 1) (tail curr-lzl)))))))
      (next-lzl 0 lzl))
  )
)
...
(define rsqrt
  (lambda (x)
    (cons-lzl x (lambda () (diag (sqrt-with x x))))
  )
)
